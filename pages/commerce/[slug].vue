<template>
  <main class="main">
    ...
    <SectionsProjectFilters
      ...
      title="помещение"
      class="project-filters_commerce"
      :count="info?.apartaments?.length"
    >
      варианты
      <span>планировок</span>
    </SectionsProjectFilters>
    <SectionsProjectChars class="project-chars_commerce" :info="info">
      <h2 class="project-chars__title h1 dark-title" style="text-align: center">
        Особенности
        <span>помещений</span>
      </h2>
    </SectionsProjectChars>
    <SectionsProjectFeatures
      class="project-dark project-features_commerce"
      :info="info.advantages"
      v-if="info.advantages?.length"
    >
      <h2 class="project-features__title h1 light-title">преимущества</h2>
    </SectionsProjectFeatures>
    <SectionsProjectPosition
      class="commerce-position"
      :info="info?.locations_map"
      v-if="info?.locations_map?.length"
      :projectCoords="info?.coords"
      :project="info?.name"
    />
    <SectionsProjectBuyWays class="buy-ways_commerce" :name="info?.name" commerce />
