  ...
  <main class="main" style="overflow: hidden">
    <div class="form-page">
      <div class="object-form__content" v-if="success">
        ...
        <ModalSucess title="Спасибо" subtitle="В ближайшее время с Вами свяжется наш специалист" />
      </div>
      <div class="object-form__content" v-else>
        <div class="object-form__header">
          <h3 class="object-form__title h1 dark-title" v-html="info?.title"></h3>
          <p class="body-text" v-html="info?.subtitle"></p>
        </div>
        <div class="object-form__main">
          <FormBrokerStarRating v-model="rating" v-if="info?.show_rating" />
          <div class="object-form__inputs">
            <FormInput placeholder="Ваше имя" v-model="name" />
            <FormPhoneInput placeholder="Ваш номер телефона" id="1231231231" v-model="phone" />
            <FormTextarea class="gc2" placeholder="Ваш комментарий" v-model="comment" />

            <div class="form-section__check" style="margin-bottom: 0" v-if="info?.policy?.url?.length">
              <FormCheckbox v-model="checked" id="form-section__check" />
              <label for="form-section__check" style="cursor: pointer">
                <span>
                  Я согласен с 
                  <NuxtLink :to="info?.policy?.url" target="_blank">
                    Согласием на обработку персональных данных
                  </NuxtLink>
                </span>
              </label>
            </div>
            <div class="form-section__check" style="margin: 0" v-if="info?.oferta?.url?.length">
              <FormCheckbox v-model="checkedSec" id="form-section__check" />
              <label for="form-section__check" style="cursor: pointer">
                <span>
                  Я согласен с 
                  <NuxtLink :to="info?.oferta?.url" target="_blank">Соглашением на оферту о сотрудничестве</NuxtLink>
                </span>
              </label>
            </div>
          </div>
