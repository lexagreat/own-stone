<template>
  ...
  <div
    class="catalog-card with-hover aft"
    :class="{
      'project-card': type == 'row',
      loading: loading,
      'full-image-hide': product?.name?.length > 19,
    }"
  >
        ...
        <ul class="catalog-card__addresses">
          ...
          <div style="min-height: 13px">
            <li v-if="product.metro_nearby?.length && product.metro_nearby[0]" style="height: 18px">
              ...
              {{ product.metro_nearby[0]?.name }},
              {{
                product.metro_nearby[0]?.time_foot
                  ? product.metro_nearby[0]?.time_foot
                  : product.metro_nearby[0]?.time_car
              }}
              мин
              <svg
                v-if="product.metro_nearby[0]?.time_foot"
              ...
              <svg
                v-if="product.metro_nearby[0]?.time_car"
          ...
          <template v-for="item in 6" :key="item">
            <li style="height: 15px">
              <p v-if="product.formatted_rooms?.length && product.formatted_rooms[item - 1]?.label">
                <span
                  v-html="
                    product.formatted_rooms[item - 1]?.label + ' ' + product.formatted_rooms[item - 1]?.formatted_area
                  "
                ></span>
              </p>
              <span v-if="product.formatted_rooms?.length && product.formatted_rooms[item - 1]?.formatted_short_price">
                {{ product.formatted_rooms[item - 1]?.formatted_short_price }}
              </span>
            </li>
          </template>
        </ul>
...
const liked = computed(() => {
  return (
    favorites.isContains(props.product?.slug, favorites.build.projects) ||
    favorites.isContains(props.product?.slug, favorites.commerce.projects)
  );
});
const link = computed(() => {
