  ...
  <div class="catalog-card catalog-card_object" @click="emit('onForm', props.product.documentId)">
    <div class="catalog-card__gallery">
      <ModalProjectPreview
        ...
        :is-open="isOpenModal"
        @close-popup="onClose"
        :photos="[product?.preview_picture]"
        :currentIndex="0"
      />
    ...
    <div class="catalog-card__main">
      <div class="catalog-card__content">
        ...
        <span class="catalog-card__price lc2" v-if="product?.type_aparts == 'Коммерция'">
          {{ product?.name_commerce ? product?.name_commerce : 'Помещение' }}, {{ product?.square_apartament }}м
          <sup>2</sup>
          , {{ product?.proekty?.name }}
        </span>
        <span class="catalog-card__price" v-else>
          {{ +product?.count_rooms > 5 ? '5+' : product?.count_rooms }} комнатная квартира,
          {{ product?.square_apartament }}м
          <sup>2</sup>
        </span>
        <ul class="catalog-card__addresses">
          ...
          <li
            v-if="
              product?.proekty?.metro_nearby?.length &&
              product?.proekty?.metro_nearby[0] &&
              (product?.proekty?.metro_nearby[0].time_foot || product?.proekty?.metro_nearby[0].time_car)
            "
          >
            ...
            {{ product?.proekty?.metro_nearby[0]?.name }},
            {{
              product.proekty.metro_nearby[0]?.time_foot
                ? product?.proekty?.metro_nearby[0]?.time_foot
                : product.proekty.metro_nearby[0]?.time_car
            }}
            мин
            <svg
              v-if="product?.proekty?.metro_nearby[0]?.time_foot"
            ...
            <svg
              v-if="product?.proekty.metro_nearby[0]?.time_car"
...
const liked = computed(() => {
  return (
    favorites.isContains(props.product?.slug, favorites.build.apartaments) ||
    favorites.isContains(props.product?.slug, favorites.commerce.apartaments) ||
    favorites.isContains(props.product?.slug, favorites.secondary)
  );
});
