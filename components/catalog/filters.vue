      ...
      <ul class="catalog-filters__items">
          ...
          <ul class="catalog-filter__categories">
            <li v-for="(item, index) in categories" :key="index">
              <input
                ...
                type="radio"
                :value="item.value"
                :id="'catalogFilterType' + item.value"
        ...
        <li class="catalog-filter" v-if="type !== 'commerce'">
          ...
          <ul class="catalog-filter__rooms">
            <li v-for="item in 6">
              <input
                v-model="roomsChecked"
                type="checkbox"
                :value="item - 1"
                :id="'catalogFilterRooms' + item"
                name="catalogFilterRooms"
              />
              <label class="circle" :for="'catalogFilterRooms' + item" v-if="item - 1 == 0">С</label>
              <label class="circle" :for="'catalogFilterRooms' + item" v-else>
                {{ item - 1 }} {{ item == 6 ? '+' : '' }}
              </label>
            </li>
          </ul>
        ...
        <li class="catalog-filter">
          ...
          <div class="catalog-range">
            <div class="catalog-range__output">
              <input
                type="text"
                :value="formatNumber(areaMinValue)"
                @input="areaMinValue = +$event.target.value.replaceAll(' ', '')"
              />
              <input
                type="text"
                :value="formatNumber(areaMaxValue)"
        ...
        <li class="catalog-filter" id="priceRange">
          ...
          <div class="catalog-range" v-if="priceType">
            <div class="catalog-range__output">
              <input
                type="text"
                :value="formatNumber(priceForOneMinValue)"
                @input="priceForOneMinValue = +$event.target.value.replaceAll(' ', '')"
              />
              <input
                type="text"
                :value="formatNumber(priceForOneMaxValue)"
            ...
            <MultiRangeSlider
              ...
              :max="priceForOneMax"
              :step="200000"
              :minValue="priceForOneMinValue"
              :maxValue="priceForOneMaxValue"
          ...
          <div class="catalog-range" v-else>
            <div class="catalog-range__output">
              <input
                type="text"
                :value="formatNumber(priceMinValue)"
                @input="priceMinValue = +$event.target.value.replace(/\D/g, '')"
              />
              <input
                type="text"
                :value="formatNumber(priceMaxValue)"
        ...
        <li class="catalog-filter" ъ>
          ...
          <UiPlacementSelect
            title="Расположение"
            :placementOptions="placementSettings.options"
            v-model:placement="placement"
            :projectsOptions="projectsSettings.options"
            v-model:projects="projects"
            :metroOptions="metroSettings.options"
            v-model:metro="metro"
          />
        </li>
        <li class="catalog-filter" v-if="!fromHome">
          ...
          <ul class="catalog-filter__options catalog-filter__rooms">
            <li v-for="item in options" :key="item">
              <input
                ...
                type="checkbox"
                :value="item.name"
                :id="'catalogFilterOptions' + item.value"
...
const search = async (map = false) => {
  ...
  if (
    (route.query['filters[category]'] == 'apartaments' && category.value == 0) ||
    (route.query['filters[category]'] == 'projects' && category.value == 1)
  ) {
    await setCat();
  }
...
const getFiltersFromQuery = () => {
  ...
  if (route.query['filters[square_apartament][$lte]'] || route.query['filters[apartaments][square_apartament][$lte]']) {
    areaMaxValue.value =
      +route.query['filters[square_apartament][$lte]'] || +route.query['filters[apartaments][square_apartament][$lte]'];
  }
  ...
  if (route.query['filters[cost_total][$lte]'] || route.query['filters[apartaments][cost_total][$lte]']) {
    priceMaxValue.value =
      +route.query['filters[cost_total][$lte]'] || +route.query['filters[apartaments][cost_total][$lte]'];
  }
  ...
  if (route.query['filters[cost_total_m2][$lte]'] || route.query['filters[apartaments][cost_total_m2][$lte]']) {
    priceForOneMaxValue.value =
      +route.query['filters[cost_total_m2][$lte]'] || +route.query['filters[apartaments][cost_total_m2][$lte]'];
  }
  if (
    route.query['filters[cost_total_m2][$gte]'] ||
    route.query['filters[apartaments][cost_total_m2][$gte]'] ||
    route.query['filters[cost_total_m2][$lte]'] ||
    route.query['filters[apartaments][cost_total_m2][$lte]']
  ) {
    priceType.value = 1;
  }
  ...
  if (route.query['filters[apartaments][finishing]']) {
    if (Array.isArray(route.query['filters[apartaments][finishing]'])) {
      ...
      repair.value[0] = repairSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][finishing]'],
      );
    }
  }
  ...
  if (route.query['filters[proekty][date_complete]']) {
    if (Array.isArray(route.query['filters[proekty][date_complete]'])) {
      ...
      dates.value[0] = dateSettings.value.options.find(
        (item) => item.name == route.query['filters[proekty][date_complete]'],
      );
    }
  }
  if (route.query['filters[apartaments][proekty][date_complete]']) {
    if (Array.isArray(route.query['filters[apartaments][proekty][date_complete]'])) {
      ...
      dates.value[0] = dateSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][proekty][date_complete]'],
      );
    }
  }
  ...
  if (route.query['filters[proekty][name]']) {
    if (Array.isArray(route.query['filters[proekty][name]'])) {
      ...
      projects.value[0] = projectsSettings.value.options.find(
        (item) => item.name == route.query['filters[proekty][name]'],
      );
    }
  }
  ...
  if (route.query['filters[apartaments][proekty][name]']) {
    if (Array.isArray(route.query['filters[apartaments][proekty][name]'])) {
      ...
      console.log(
        projectsSettings.value.options.find((item) => item.name == route.query['filters[apartaments][proekty][name]']),
      );
      projects.value[0] = projectsSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][proekty][name]'],
      );
    }
  }
  ...
  if (route.query['filters[proekty][metro_nearby][name]']) {
    if (Array.isArray(route.query['filters[proekty][metro_nearby][name]'])) {
      ...
      metro.value[0] = metroSettings.value.options.find(
        (item) => item.name == route.query['filters[proekty][metro_nearby][name]'],
      );
    }
  }
  ...
  if (route.query['filters[apartaments][proekty][metro_nearby][name]']) {
    if (Array.isArray(route.query['filters[apartaments][proekty][metro_nearby][name]'])) {
      ...
      metro.value[0] = metroSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][proekty][metro_nearby][name]'],
      );
    }
  }
  ...
  if (route.query['filters[apartaments][floor]']) {
    if (Array.isArray(route.query['filters[apartaments][floor]'])) {
      ...
      floor.value[0] = floorSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][floor]'],
      );
    }
  }
  ...
  if (route.query['filters[proekty][district]']) {
    if (Array.isArray(route.query['filters[proekty][district]'])) {
      ...
      placement.value[0] = placementSettings.value.options.find(
        (item) => item.name == route.query['filters[proekty][district]'],
      );
    }
  }
  if (route.query['filters[apartaments][proekty][district]']) {
    if (Array.isArray(route.query['filters[apartaments][proekty][district]'])) {
      ...
      placement.value[0] = placementSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][proekty][district]'],
      );
    }
  }
  ...
  if (route.query['filters[ring_road]']) {
    if (Array.isArray(route.query['filters[ring_road]'])) {
      ...
      transport.value[0] = transportSettings.value.options.find(
        (item) => item.name == route.query['filters[ring_road]'],
      );
    }
  }
  if (route.query['filters[apartaments][ring_road]']) {
    if (Array.isArray(route.query['filters[apartaments][ring_road]'])) {
      ...
      transport.value[0] = transportSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][ring_road]'],
      );
    }
  }
  ...
  if (route.query['filters[apartaments][appointment]']) {
    if (Array.isArray(route.query['filters[apartaments][appointment]'])) {
      ...
      target.value[0] = targetSettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][appointment]'],
      );
    }
  }
  ...
  if (route.query['filters[apartaments][entrance]']) {
    if (Array.isArray(route.query['filters[apartaments][entrance]'])) {
      ...
      entry.value[0] = entrySettings.value.options.find(
        (item) => item.name == route.query['filters[apartaments][entrance]'],
      );
    }
  }
