<template>
  <UiModal :isOpen="isOpen" @closePopup="emit('closePopup')" class="object-form">
    <div class="modal__content object-form__content" v-if="success">
      ...
      <ModalSucess title="Заявка <span>отправлена</span>" subtitle="В ближайшее время с Вами свяжется наш специалист" />
    </div>
        ...
        <div class="object-form__inputs">
          ...
          <div class="text-input">
            <input
              ...
              @input="budget = +$event.target.value.replaceAll(' ', '')"
            />
          </div>
...
const isDisabledBtn = computed(() => {
  return checked.value && fio.value.length && phone.value.length >= 15 && budget.value;
});
...
const send = async () => {
  ...
  let mailSenderObject = {
    subject: 'Новый клиент с сайта Own stone',
    text: `
         ФИО: ${fio.value}
         Телефон: ${wrapper.value.querySelector('.phone p span').innerHTML} ${phone.value}
         Бюджет: ${budget.value}
         тип: ${typeOption.value.name}
      `,
  };
  let response = await store.addNewClient(object);
  // console.log(response);
  if (response.status) {
    let mailSenderResponse = await store.sendForm(mailSenderObject);
    fio.value = '';
    phone.value = '';
    budget.value = '';
    checked.value = false;
