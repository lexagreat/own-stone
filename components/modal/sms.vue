  ...
  <UiModal @closePopup="emit('closePopup')" class="auth-modal">
    <div class="auth-modal__content modal__content">
      ...
      <div class="auth-modal__header">
        <h3 class="auth-modal__title h1 dark-title">
          ВВЕдите
          <br class="pc" />
          <span>СМС-код</span>
        </h3>
      ...
      <div class="sms-modal__main">
        ...
        <span v-if="error" style="color: #ff5e5c; display: block; margin-top: -20px">Неверный код</span>
        <span v-if="store.time > 0">
          Получить код повторно через 00:{{ store.time >= 10 ? store.time : '0' + store.time }}
        </span>
