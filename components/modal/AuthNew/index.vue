<template>
  <UiModal @closePopup="emit('closePopup'), (success = false)" class="auth-modal auth-new" :class="{ reg: tab == 0 }">
    ...
    <div class="auth-modal__content modal__content" v-else>
      ...
      <h3 class="auth-modal__title h1 dark-title" v-if="tab == 1">
        вход в личный
        <span>кабинет</span>
      </h3>
      <div class="auth-modal__main">
        <ul class="auth-modal__list" v-if="tab == 0">
          ...
          <li class="gc2">
            <UiSelect class="app-select auth-select" :settings="formSettings" @selectOption="onSelectFormOption" />
          </li>
        </ul>
        ...
        <div class="form-section__check" style="margin: 0">
          <FormCheckbox v-model="checkedSec" id="form-section__check2" />
          <label for="form-section__check2" style="cursor: pointer">
            <span>
              Я согласен с 
        ...
        <div class="form-section__check" style="margin: 0" v-if="tab == 0">
...
const isFormValid = computed(() => {
  return (
    ...
    store.time <= 0 &&
    (tab.value === 1 ||
      (tab.value === 0 && name.value.length > 0 && surname.value.length > 0 && validEmail(email.value)))
  );
});
const register = async () => {
  ...
  let res = await store.getCode(
    document.querySelector('.auth-new .auth-modal__main .phone p span').innerHTML + ' ' + phone.value,
    obj,
  );
  if (!res.status) {
...
watch(
  () => [name.value, surname.value, email.value, phone.value, form.value, tab.value],
  (newValues) => {
    errorMes.value = '';
  },
  { deep: true },
