<template>
  ...
  >
    ...
    <yandex-map-marker
      ...
      v-if="JSON.parse(projectMarker?.coords)"
      :settings="{ coordinates: JSON.parse(projectMarker?.coords) }"
    ...
    <yandex-map-clusterer :settings="{}">
      <template v-for="(item, index) in info" :key="index">
        <yandex-map-marker
          position="top-center left-center"
          v-if="JSON.parse(item?.coords) && item?.id"
          :settings="{ coordinates: JSON.parse(item?.coords) }"
          @click="toggleBalloon(index)"
        >
