  ...
  actions: {
    ...
    async getCode(phone, object) {
      let response = await useBaseFetch(
        `/profile?type=${object.type}&phone=${phone}&firstname=${object.firstname}&secondname=${object.secondname}&email=${object.email}&user_avctivity=${object.user_avctivity}&advertisingcheckmark=${object.advertisingcheckmark}`,
      );
      return response;
    },
    async sendCode(phone, code) {
      let response = await useBaseFetch(`/profile?type=confirm_code&phone=${phone}&code=${code}`);
      if (response?.status) {
      ...
      if (response?.id) {
        ...
        if (this.user?.clients?.length > 1) {
          this.user.clients = this.user.clients.filter(
            (item, index, self) => index === self.findIndex((t) => t?.documentId === item?.documentId),
          );
        }
      }
      return response;
